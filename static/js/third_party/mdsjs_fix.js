...
mdsjs = function() {
  ...
  this.landmarkMDSAsync = function(dist, dims, cb, argCall) {
    ...
    function landmarkResult(dist, dims, eigenVecs, eigenVals) {
      ...
      var tmp = eigenVecs.createArray(eigenVecs.rows, eigenVecs.columns);
      var pos = 0;
  ...
  this.pivotRandom = function(m, k) {
    if(!m.isQuadratic()) {
      console.warn("quadratic matrix needed", m.rows, m.columns);
      return null;
    }
    if(k < m.rows) {
      console.warn("requested more pivots than elements", k, m.rows, m.columns);
      return null;
    }
    var mat = m.createArray(k, m.columns);
    var pivots = {};
    var pos = 0;
    for(var i = 0;i < k;i += 1) {
      var pivot = 0;
      do {
        pivot = Math.random() * m.columns;
      } while(pivots[pivot]);
      pivots[pivot] = true;
      for(var c = 0;c < m.columns;c += 1) {
        mat[pos] = m.distance(pivot, c);
        pos += 1;
      }
    }
    return new Matrix(mat, k, m.columns);
  };
  ...
  Matrix.prototype.neg = function() {
    var mat = this.createArray(this.rows, this.columns);
    for(var pos = 0;pos < mat.length;pos += 1) {
      mat[pos] = -this.getUnsafe(pos);
    }
    return new Matrix(mat, this.rows, this.columns);
  };
  ...
  Matrix.iter = function(matA, matB, row, col, cb) {
    if(matA.columns !== matB.rows) {
      console.warn("incompatible dimensions", matA.rows + "x" + matA.columns, matB.rows + "x" + matB.columns);
      return;
    }
    var posA = row * matA.columns;
    var posB = col;
    for(var i = 0;i < matA.columns;i += 1) {
  ...
  Matrix.mul = function(matA, matB) {
    ...
    var mat = matA.createArray(matA.rows, matB.columns);
    for(var r = 0;r < matA.rows;r += 1) {
      matA.rowIter(r, function(a, _, k) {
        var pos = r * matB.columns;
    ...
    return new Matrix(mat, matA.rows, matB.columns);
  };
  Matrix.add = function(matA, matB) {
    ...
    var mat = matA.createArray(matA.rows, matA.columns);
    for(var pos = 0;pos < mat.length;pos += 1) {
      mat[pos] = matA.getUnsafe(pos) + matB.getUnsafe(pos);
    }
    return new Matrix(mat, matA.rows, matA.columns);
  };
