...
/**
 * Creates new PCA (Principal Component Analysis) from the _dataset
 * @param {Matrix} _dataset - _dataset or covariance matrix
 * @param {Object} options
 * @param {boolean} [options.isCovarianceMatrix=false] - true if the _dataset is a covariance matrix
...
class PCA {
    constructor(dataset, options) {
        ...
        if (options.isCovarianceMatrix) { // user provided a covariance matrix instead of _dataset
        ...
        if (useCovarianceMatrix) { // user provided a _dataset but wants us to compute and use the covariance matrix
    ...
    /**
     * Project the _dataset into the PCA space
     * @param {Matrix} dataset
     * @return {Matrix} _dataset projected in the PCA space
     */
    predict(dataset) {
        ...
        if (this.center) {
            ...
            if (this.scale) {
                for (var i = 0; i < stdevs.length; i++) {
                    if (stdevs[i] === 0) {
                        throw new RangeError('Cannot scale the _dataset (standard deviation is zero at index ' + i);
                    }
                }
                this.stdevs = stdevs;
